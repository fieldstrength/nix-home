''

snippet /rm "Normal text" iA
\\mathrm{$1}$0
endsnippet

snippet /bf "Bold" iA
\\mathbf{$1}$0
endsnippet

snippet /bb "Blackboard Bold text" iA
\\mathbb{$1}$0
endsnippet

snippet /cal "Calligraphy text" iA
\\mathcal{$1}$0
endsnippet

snippet /frak "Fraktur text" iA
\\mathfrak{$1}$0
endsnippet

snippet /scr "Script text" iA
\\mathscr{$1}$0
endsnippet


snippet /// "Fraction" iA
\\frac{$1}{$2}$0
endsnippet

snippet //t "Tiny fraction" iA
\\tfrac{$1}{$2}$0
endsnippet

snippet oxx "Tensor times" iA
 \\otimes $0
endsnippet

snippet /\ "Wedge product" iA
 \\wedge $0
endsnippet

snippet /int "Integral" iA
\\int_{$1}^{$2} $0
endsnippet

snippet /sum "Sum" iA
\\sum_{$1}^{$2} $0
endsnippet

snippet /inf "Infinity" iA
\\infty$0
endsnippet

snippet /. "Compose" iA
 \\circ $0
endsnippet

snippet /sq "Square-root" iA
\\sqrt{$1}$0
endsnippet

snippet /vec "Vector 3D" iA
\\vec{$1}$0
endsnippet

snippet /mat "Matrix" iA
\begin{matrix}
$1
\end{matrix} $0
endsnippet

snippet /> "Natural transformation" iA
 \\Longrightarrow $0
endsnippet

snippet /over "Overtext" iA
\\overtext{$1}{$2} $0
endsnippet

snippet /dd "partial derivative" iA
\\partial$0
endsnippet

snippet //p "partial derivative fraction" iA
\\frac{\\partial $1}{\\partial $2} $0
endsnippet

snippet //d "ordinary derivative fraction" iA
\\frac{\\partial $1}{\\partial $2} $0
endsnippet

snippet /del "Nabla" iA
\\nabla$0
endsnippet


snippet /ket "ket" iA
\\lvert $1 \\rangle$0
endsnippet

snippet /bra "bra" iA
\\langle $1 \\rvert$0
endsnippet

snippet /xpect "expectation value" iA
\\langle $1 \vert $2 \vert $1 \\rangle$0
endsnippet

snippet /inner "inner product" iA
\\langle $1 \vert $2 \\rangle $0
endsnippet

snippet /ev "expectation value with implicit states" iA
\\langle $1 \\rangle$0
endsnippet



snippet /pmat "matrix" iA
\\begin\{pmatrix\}
$1 \\\\ $2
\\end\{pmatrix\}
endsnippet


snippet /box "boxed equation" iA
\$\$
\\bbox[8px,border:2px solid #DAB5E5]\{
$1
\}
\$\$
endsnippet


snippet ( "Cheeky open parentheses" iA
($1)$0
endsnippet

snippet [ "Cheeky open bracket" iA
[$1]$0
endsnippet

snippet { "Cheeky open brace" iA
\{$1\}$0
endsnippet


# Raise priority to prevent issues with cheeky parentheses
priority 1

snippet /( "Open parentheses" iA
\\Big( $1 \\Big)$0
endsnippet

snippet /b( "Big open parentheses" iA
\\bigg( $1 \\bigg)$0
endsnippet

snippet /[ "Open bracket" iA
\\Big[ $1 \\Big]$0
endsnippet

snippet /b[ "Big open bracket" iA
\\bigg[ $1 \\bigg]$0
endsnippet

snippet /{ "Open curly brace" iA
\\big\\\{ $2 \\big\\\}$0
endsnippet

snippet /b{ "Big open curly brace" iA
\\bigg\\\{ $1 \\bigg\\\}$0
endsnippet


snippet /| "Open absolute value" iA
\\Big | $1 \\Big|$0
endsnippet

''
